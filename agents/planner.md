...
2. **Produce a plan** with these sections:
   ...
   - **API surface** — route handlers and/or server actions, their Zod input
     schemas, response DTOs, and error cases.
   - **Services** — the functions in `modules/<feature>/services/` that own the
     logic, with signatures.
   - **UI** — pages/components needed, server vs client components, loading and
     error states.
